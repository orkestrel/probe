		...
		try {
			...
			scratch.write(
				...
				[
					...
					`const claim = ${claim}`,
					'const probe = server.createProbe({ workspace: process.cwd(), deadline: 60_000 })',
					'try {',
					'\tconst verdict = await probe.prove(claim)',
					"\tif (typeof verdict.receipt !== 'string' || verdict.checks.length !== 3 || verdict.control.length !== 3) throw new Error('The ESM probe returned no verdict')",
					'} finally {',
					'\tawait probe.destroy()',
					'}',
				].join('\n'),
			...
			scratch.write(
				...
				[
					...
					'\ttry {',
					'\t\tconst verdict = await probe.prove(claim)',
					"\t\tif (typeof verdict.receipt !== 'string' || verdict.checks.length !== 3 || verdict.control.length !== 3) throw new Error('The CommonJS probe returned no verdict')",
					'\t} finally {',
					'\t\tawait probe.destroy()',
					'\t}',
