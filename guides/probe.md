...
| `Toolchain`         | interface | `{ typescript, oxlint, vitest }` — the versions each tool's own manifest publishes in the target workspace, read from those manifests rather than from the module a stage loaded.           |
...
| `isToolchain` | function | `(value: unknown) => value is Toolchain` | Admits a record carrying every tool version the target workspace's installed manifests publish.                                                                |
...
| `loadWorkspaceModule`      | function | `(workspace: string, specifier: 'typescript') => typeof import('@typescript/typescript6')` and `(workspace: string, specifier: 'vitest/node') => typeof import('vitest/node')` | Loads one installed tool module, falling back to the workspace's `@typescript/typescript6` for a `typescript` that carries no in-process compiler API, or throws a `workspace` failure carrying the native fault as `cause` where one was raised.                                                                                                                                                                                                                                                                                                                                             |
...
| `collectRangeMajors`       | function | `(range: string) => readonly string[]`                                                                                                                                         | Collects every major version a caret range names, skipping a term that is not one caret version.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `workspace`  | `malformed` | The target tree publishes something probe cannot read: an unparsable manifest, a `bin` entry that is not a path, a TypeScript project its own compiler refuses, a `typescript` carrying no in-process compiler API with no `@typescript/typescript6` able to serve one beside it, an unsupported tool version, a mutation path whose existing components cannot be inspected, or a directory it blocks probe from creating for the boot workbench.                    |
...
- **The workspace's `typescript`, `oxlint`, and `vitest` are the same resolved files probe
  resolves.** probe resolves each of them from the target workspace, never from its own
  dependencies, and reports the version each tool's own installed manifest publishes on
  `Verdict.toolchain`. A verdict predicts the gate only while probe and the gate read one installed
  copy of each tool.
- **That `typescript` carries the in-process compiler API, or `@typescript/typescript6` sits beside
  it.** The type stage drives the compiler inside this process, and TypeScript 7 publishes no
  in-process compiler API under the `typescript` specifier. `loadWorkspaceModule` returns the
  workspace's `typescript` where it exports `createProgram` as a function, and the workspace's
  `@typescript/typescript6` where it does not — the resolution `unplugin-dts` performs for the same
  reason. Install that bridge in a workspace on TypeScript 7: `npm install -D @typescript/typescript6`.
  A bridged workspace's type stage runs the 6.x compiler the bridge republishes rather than the
  workspace's own 7.x, so a type verdict predicts that workspace's gate only where the two compilers
  agree. probe's own declarations import the bridge's types, so a consumer that typechecks against
  them with `skipLibCheck` off installs the bridge too.
  A workspace whose `typescript` carries no in-process compiler API and whose
  `@typescript/typescript6` cannot serve one is refused with `origin: 'workspace'` and
  `code: 'malformed'`:
  ...
  A workspace installing no `typescript` at all keeps the `missing` refusal,
  `The workspace cannot load typescript`.
  `Probe` reads every major its own `typescript` peer range names, through `collectRangeMajors`, and
  accepts a workspace whose installed major is one of them, so a TypeScript 7 workspace carrying the
  bridge is served rather than refused. A major the range names nowhere is refused at `prove` with
  `The supported TypeScript range is <range>; found <version>`.
  `Verdict.toolchain` reports the version each tool's own manifest publishes in the target
  workspace, so `toolchain.typescript` reads that workspace's `typescript` version whatever engine
  served the type stage: a bridged workspace reports its `7.x`, never the 6.x compiler the bridge
  republishes.
...
verdict.receipt // 'probe:0806fb30f428edb8ea85adfb4b355441:type:typescript@6.0.3:oxlint@1.81.0:vitest@4.1.11:configs/src/tsconfig.core.json@3b674fdf121c85efb9ed1bab25ceeec8'
await probe.destroy()
```
...
- A tool field per tool follows, spelled `<name>@<version>`, in the order `typescript`, `oxlint`,
  `vitest`. Each version is the one that tool's own installed manifest publishes in the target
  workspace, so a bridged workspace's token names its `typescript` 7.x while the 6.x compiler the
  bridge republishes judged the code.
