# CLEAN-PROBE — delete every count from this package's prose
...
Delete every count and every positional reference from `/workspace/probe`'s prose, so no sentence anywhere in
...
The rule now reads, in `/workspace/probe/AGENTS.md` § Writing:
...
Read before acting: `/workspace/probe/AGENTS.md` § Writing, `/workspace/probe/.claude/rules/writing.md`, and
`/workspace/probe/.claude/rules/documentation.md`.

Host: Linux container, bash. `/workspace/probe` is a clean checkout. `npx scaffold audit` resolves normally here. The `src:server` and `src:bin` projects each take roughly 500 seconds.
